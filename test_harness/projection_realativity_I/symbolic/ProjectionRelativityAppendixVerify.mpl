...
local main, supp, mainText, suppText;
    ...
    if not main[1] then
        if PR_require_source_text then
            error "Required repo-local main paper not found: %1. Run from the repository root or test_harness folder in a checkout containing manuscript/ after updating the repository, for example with git pull.", PR_source_path;
        else
    ...
    if not supp[1] then
        if PR_require_source_text then
            error "Required repo-local supplement not found: %1. Run from the repository root or test_harness folder in a checkout containing manuscript/ after updating the repository, for example with git pull.", PR_supplement_path;
        else
...
local fd, i, row;
    ...
    fprintf(fd, "# Projection Relativity Equation Map\n\n");
...
local fd, i, row;
    ...
    fprintf(fd, "# Projection Relativity Core Equation Derivations\n\n");
...
local fd, i, row;
    ...
    fprintf(fd, "# Projection Relativity Maple Proof Report\n\n");
...
local fd, i, row;
    ...
    fprintf(fd, "# Projection Relativity Independent Spectrum Recompute\n\n");
...
local fd, i, row;
    ...
    fprintf(fd, "# Projection Relativity Harness Self-Validation\n\n");
...
local fd;
    ...
    fprintf(fd, "# Projection Relativity Equation Audit\n\n");
...
global PR_passes, PR_notes, PR_assumptions, PR_data_requirements,
       PR_manuscript_issues, PR_method_boundaries, PR_selftest_passes,
       PR_spectrum_rows, PR_proof_rows;
    PR_passes := 0;
    PR_notes := 0;
    PR_assumptions := 0;
    PR_data_requirements := 0;
    ...
    PR_WriteEquationMap("equation_map.md");
    PR_WriteHarnessValidation("harness_validation.md");
    PR_WriteDerivationReport("equation_derivations.md");
    PR_WriteProofReport("proof_report.md");
    PR_WriteSpectrumReport("spectrum_recompute.md");
    PR_WriteEquationAudit("equation_audit.md");
end proc:
