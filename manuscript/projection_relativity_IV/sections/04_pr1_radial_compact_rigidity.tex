...
This section proves rigidity in the completed PR-I admissible class and assigns
a nonzero admissibility defect to every inequivalent in-class value.
...
\end{equation}
Thus the associated one-dimensional Schr\"odinger operator is confining and
has a lower-bounded discrete simple spectrum on its standard self-adjoint
domain \cite{ReedSimon_1975}.

Equation~\eqref{eq:pr35-radial-unique} is unique in the class
\(\mathfrak V_4\).  Higher even or nonpolynomial terms violate the declared
minimal-even-quartic rule and define a different operator class; they are not
additional PR radial solutions.
...
Let the odd radial eigenbasis be
\(\{\phi_1,\phi_3,\phi_5,\ldots\}\).  Require a coordinate spectral
projector that contains both the fundamental odd winding-linked channel
\(\phi_1\) and the spatial primitive-return channel \(\phi_3\), preserves
parity, and has minimal rank.  There is exactly one rank-two coordinate
projector satisfying these requirements:
\begin{equation}
\boxed{
\Pi_{13}
=
...
\end{equation}
The theorem domain requires channels (1) and (3), as fixed by their
fundamental winding-linked and primitive spatial-return roles.  The
minimal-channel principle is the canonical PR-I closure: every added odd
channel violates minimal rank and lies outside the admissible class.
...
\label{eq:pr35-pgeom}
\end{equation}
On the range of \(\Pi_{13}\), Eq.~\eqref{eq:pr35-pgeom} is a real
symmetric $2\times2$ matrix.  Its reconstructed off-diagonal element is
nonzero and its eigenvalues are distinct.  The dominant normalized eigenvector
is therefore unique up to an overall sign.  The sign cancels from
\begin{equation}
p_1
=
...
\subsection{PR-I Closure Theorem}
...
compact stiffness are numerically isolated and reproducible.  The
minimal-quartic function class, primitive return set \(\{A^3,A^4\}\), required
channel identification \(\{\phi_1,\phi_3\}\), and finite-rank orientation rule
producing \(-XA^8\) are the declared minimal closure conditions.  They define
the complete PR-I theorem domain used in this paper.  Within that domain, the
radial and compact-boundary equation quotient is a singleton.
