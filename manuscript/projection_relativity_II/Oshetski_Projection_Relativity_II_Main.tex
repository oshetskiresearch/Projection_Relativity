...
\hypersetup{
  pdftitle={The Foundation of Projection Relativity II:
  Geometric Constraints on the Non-Abelian Phase Fiber},
  pdfauthor={Michael Stanislaus Oshetski},
  pdfsubject={Projection Relativity II: electroweak and flavor closure},
  pdfkeywords={projection relativity, electroweak theory, flavor,
  compact geometry, CKM, PMNS, neutrino masses}
}
...
\maketitle
\begin{abstract}
This manuscript extends the projection architecture introduced in Projection Relativity I (PR-I) to the microscopic electroweak and flavor sectors. The central question is whether the particle ledger, usually treated as a collection of independent empirical inputs, can instead be organized as a compact projection structure with testable boundary outputs. We investigate the parent compact non-Abelian phase space
\[
...
SU(2)_L,
\]
and evaluate the conditions under which this parent fiber locks into the residual electromagnetic boundary. In this formulation, the photon is the unbroken residual projection, while the \(W^\pm\) and \(Z\) modes arise as massive projection excitations of the broken electroweak directions. Within this lifted compact fiber, the standard electroweak charge assignments, charged-current ladder structure, neutral-current \(Z\)-couplings, Fermi limit, and tree-level relation \(\rho_{\rm EW}=1\) are recovered as projection-locking consequences. We then explore the boundary deficit as a geometric source for the weak mixing angle and electroweak scale, obtaining a zero-new-parameter closure candidate for \(\sin^2\theta_W\), \(v_{\rm EW}\), and \(G_F\). The same boundary ledger naturally yields a three-generation projection structure,
\[
N_{\rm gen}^{\rm PR}
=
\dim\!\left[
...
3.
\]
By evaluating sector-specific spectral overlaps across these projection sheets, we obtain charged-lepton, quark, CKM, PMNS, and neutrino mass-sector candidates without using the corresponding target observables as generation inputs. The charged-lepton sector gives a precision Koide-sheet closure candidate; the quark sector yields a compact-boundary hierarchy with PR-derived heavy-quark thresholds and QCD running anchors; and the neutral-lepton sector selects a normal-ordering, Majorana-dominant candidate. This neutrino branch produces a testable boundary prediction for the neutrinoless double-beta decay effective Majorana mass,
\[
m_{\beta\beta}^{\rm PR}
\simeq
1.508\ {\rm meV},
\]
placing the PR-II neutrino sector near the lower-interference edge of the normal-ordering Majorana envelope. Throughout, target measurements are used only as diagnostic references after the projection formulas have generated their values. The result is not presented as a completed electroweak precision theorem, but as a test-driven, zero-new-parameter boundary closure program for organizing the electroweak and flavor structure of the Standard Model from projection geometry.
\end{abstract}
...
\end{equation}
Here \(S_Y^1\) is the compact hypercharge phase and \(SU(2)_L\) is the compact non-Abelian weak-isospin fiber. The electromagnetic sector derived in PR-I is not altered. Instead, it is recovered as the residual locked boundary of the parent electroweak fiber:
\begin{equation}
\boxed{
SU(2)_L\times U(1)_Y
\longrightarrow
...
\end{equation}
In this interpretation, the photon is the unbroken residual projection mode, while \(W^\pm\) and \(Z\) are massive projection excitation modes associated with the broken electroweak directions.

\subsection{From Electroweak Projection to Flavor Ledger}
\label{sec:intro_pr2_program}

The first part of PR-II establishes the non-Abelian phase projection itself. The \(SU(2)_L\) generators obey $[T_a,T_b] = i\epsilon_{abc}T_c$, and the weak charged-current ladder is generated by $T^\pm = T_1\pm iT_2.$
...
The second part of the manuscript asks whether the numerical electroweak scales can be tied to boundary data. The weak mixing angle is treated as a boundary-deficit effect. The electroweak scale is then generated through a spectral hierarchy action built from the PR-I boundary constants and the reduced Planck scale. This produces a zero-new-parameter weak-scale candidate:
\begin{equation}
\boxed{
v_{\rm EW}^{\rm PR}
=
...
\end{equation}
This provides a geometric origin candidate for the three-generation ledger. Sector-specific overlap operators are then constructed for charged leptons, quarks, and neutrinos. The charged-lepton sector yields a Koide-sheet precision candidate \cite{Koide1983}; the quark sector yields heavy-threshold and light-quark hierarchy candidates; and the neutral-lepton sector yields a normal-ordering Majorana-dominant neutrino candidate.

\subsection{Scope of the Claims}
\label{sec:intro_scope}

To keep the status of each result clear, the claims in this manuscript are grouped into four categories.

\textbf{Structural identities} are results that follow directly from the algebra or representation content of the construction. These include the \(SU(2)_L\) generator algebra, the residual electromagnetic charge operator
\begin{equation}
Q=T_3+\frac{Y}{2},
\end{equation}
the massless photon eigenmode, the tree-level \(W/Z\) mass relations, the Fermi matching condition, the tree-level scalar-coupling ledger, and anomaly cancellation for the chiral fermion representation \cite{Adler1969,Witten1982}.

\textbf{Closure candidates} are boundary relations proposed to connect the inherited PR-I boundary data to quantities that are usually treated as independent inputs. This category includes the weak-mixing relation, the strong-coupling anchor, the three-generation projection rule, the sector-specific overlap operators, the CKM and PMNS constructions, and the neutrino mass-splitting hierarchy.

\textbf{Precision candidates} are closure relations whose generated values can be compared directly with high-precision reference data. These include the weak-scale construction, the charged-lepton Koide-sheet closure \cite{Koide1983}, and the heavy-quark threshold-scale candidates. The label ``precision candidate'' refers to the numerical agreement of the construction; it does not imply that radiative corrections, renormalization-scheme dependence, or higher-order matching have already been completed.

\textbf{Diagnostic ledgers} provide the running, matching, and comparison framework needed to interpret the generated quantities. These include QCD threshold matching, higher-loop running, electroweak running conventions, and comparisons with reference data from the electroweak, flavor, quark, and neutrino sectors. Reference values are introduced only after the PR-II formulas have generated their outputs.

PR-II is therefore not presented as a completed electroweak precision theory. It is presented as a test-driven compact-boundary closure program whose assumptions, generated outputs, diagnostic comparisons, and remaining derivation obligations are stated separately. The central claim is narrower: a substantial part of the tree-level electroweak and flavor ledger can be organized from the PR boundary data without fitting the target observables directly.
...
The PR-II electroweak/flavor chain is supported by step-specific validation scripts and more than six hundred automated checks. The harness ledger verifies the compact electroweak lift, projection locking, weak mixing, weak-scale closure, charged and neutral currents, anomaly cancellation, generation replication, flavor hierarchy, charged-lepton precision closure, QCD running, heavy-quark thresholds, CKM and PMNS mixing, and the neutrino mass-sector candidate. Target observables such as \(G_F\), \(v_{\rm EW}\), \(M_W\), \(M_Z\), charged-lepton masses, quark masses, CKM angles, PMNS angles, \(\alpha_s(M_Z)\), and neutrino mass splittings are not used as generation inputs. They enter only as diagnostic references. The code and generated outputs are maintained in the public repository:
...
\end{equation}
and the boundary cofactor
\begin{equation}
c_{\rm bc} = \frac{3354902985}{4211081216} = 0.796684464847899.
\end{equation}
Together they reproduce the residual compact electromagnetic normalization\footnote{\textbf{Note on numerical resolution and framework scope.}
The numerical constants used throughout PR-II are inherited from the published finite-resolution PR-I ledger so that every downstream result remains reproducible from the original release. Independent banded harmonic-basis calculations stabilize the low radial spectrum and finite-rank boundary map at $\mu_{\min}^2\simeq 3.053016666733$,\quad $\alpha_{\rm PR,bc}^{-1}\simeq 137.036041314016$. This reduces the electromagnetic residual from approximately \(2.65\,{\rm ppm}\) in the original ledger to approximately \(0.31\,{\rm ppm}\) relative to the CODATA recommended value. The refinement demonstrates numerical stability and substantially improved agreement. The refined constants are not mixed into the present PR-II calculations. Their consistent propagation is reserved for a separate high-resolution release.},
\begin{equation}
\boxed{
\alpha_{\rm PR,bc}^{-1}
=
...
Instead, PR-II asks whether that residual \(U(1)_{\rm em}\) boundary is the locked remnant of a larger compact electroweak fiber. The determinant suppression ratio inherited from PR-I is
\begin{equation}
r_G
=
...
\end{equation}
The normalized boundary deficit is
\begin{equation}
\boxed{
\varepsilon_{\rm bc}
=
...
\end{pmatrix}.
\end{equation}
Consequently, the conventional electromagnetic charges emerge from the compact projection-locking relation as described in equation ~\ref{equ:residual_charge}. Translated into the language of PR-II, the photon operates as the unbroken residual compact projection mode. Furthermore, the \(W^\pm\) and \(Z\) modes are no longer treated as unrelated, primitive objects; instead, they manifest as massive projection excitation modes associated with the broken directions of the parent electroweak fiber. This interpretation preserves the standard low-energy electroweak ledger, replacing independent parameter insertion, where possible, with compact-boundary closure tests.

\subsection{Input Discipline}
\label{sec:pr2_boundary_input_discipline}

The purpose of PR-II is to test whether the electroweak and flavor ledger can be generated from compact projection data without introducing new fitted microscopic parameters. Accordingly, the following quantities are not allowed as generation inputs in the closure chain \cite{TiesingaCODATA2021, PDG2024}:
...
\end{equation}
In PR-II language, the \(j=0\) sector is weak-isospin neutral, the \(j=1/2\) sector supplies the compact weak doublet ledger, and the \(j=1\) sector supplies the compact weak projection modes. This spectral hierarchy is the microscopic origin of the weak-sector distinction between matter doublets and gauge excitation directions. 
...
\end{equation}
After projection locking, these ladder moves become the charged-current weak processes mediated by the massive projection excitation modes \(W^\pm\).
...
\end{equation}
The later projection-locking condition determines which combination remains massless and which combinations become massive projection excitation modes.
...
The charged projection modes are
\begin{equation}
W_\mu^\pm
=
\frac{1}{\sqrt2}
...
\end{equation}
Projection locking therefore produces one massless residual electromagnetic mode and three massive weak projection excitation modes.
...
$e=g\sin\theta_W=g'\cos\theta_W$. The photon \(A_\mu\) is the unbroken \(Q\)-projection mode. The \(Z_\mu\) is the neutral projection excitation orthogonal to the residual electromagnetic boundary. The neutral projection-locking split is $(W_\mu^3,B_\mu) \longrightarrow (A_\mu,Z_\mu)$, with
$M_A=0, \space M_Z>0$.
...
It is necessary to verify that its unbroken geometric generator correctly governs the matter sector. The electromagnetic charges are derived from the projection-locking relation. The unbroken residual generator is dictated by equation ~\ref{equ:residual_charge}. Evaluating this unbroken direction naturally recovers the standard Standard Model charge assignments \cite{PDG2024}. For the left-handed lepton doublet, the residual electromagnetic generator recovers the standard charge assignments. For the left-handed lepton doublet,
\begin{equation}
\boxed{
L_L
=
...
Projection locking converts the parent compact electroweak fiber into a residual electromagnetic boundary plus massive weak projection excitations:
\begin{equation}
\boxed{
SU(2)_L\times U(1)_Y
\longrightarrow
...
\end{equation}
In PR-II language, \(W^\pm\) and \(Z\) are not primitive independent additions to the theory.  They are the massive excitation modes of the compact electroweak fiber after the residual electromagnetic boundary has been selected. The photon remains massless because it is the unbroken \(Q\)-projection mode. The projection-locking chain is therefore
\begin{equation}
\boxed{
\mathcal M_{\rm int}^{\rm EW}
\rightarrow
...
\subsection{Falsification Bounds}
...
\det M_{0,\rm wrong}^2\neq0,
\end{equation}
resulting in a massive photon. Such a disconnected structure is rejected by the compact electroweak lift. Second, if the unbroken residual generator deviated from the precise locking direction equation ~\ref{equ:residual_charge}, the locked vacuum would fail to be neutral, and the standard electromagnetic charge assignments would not be recovered, instantly breaking the low-energy effective ledger. Third, if the weak mixing angle were dictated by the raw hypercharge trace,
\begin{equation}
\sin^2\theta_W=\frac14,
\end{equation}
...
\end{equation}
The electroweak scale must then arise from a dimensionless spectral suppression:
\begin{equation}
\Lambda_{\rm EW}^{\rm PR}
=
\bar M_{\rm Pl}
...
The final term is the weak boundary-deficit correction averaged over the residual compact boundary. The leading action gives
\begin{equation}
\boxed{
S_{\rm EW}^{\rm PR,base}
=
...
We introduce the following second-order compact-boundary closure term 
\begin{equation}
\delta S_{\rm EW}^{(2)}
=
-\frac12
...
\frac{1+3c_{\rm bc}}{2}
\end{equation}
combines one residual electromagnetic singlet contribution with three weak-adjoint contributions under the normalization adopted in this closure rule. It is referred to below as the singlet-plus-adjoint compact factor. The numerical effect of the second-order term is to refine the leading weak-scale candidate. Its independent derivation and
uniqueness remain part of the PR-III precision future research. The corrected hierarchy action is
\begin{equation}
\boxed{
...
\label{sec:pr2_generated_weak_scale_fermi_constant}
 The compact hierarchy now provides a candidate weak scale and the corresponding Fermi constant. These quantities are evaluated from the inherited boundary ledger and fundamental constants, while experimental values are introduced only afterward for diagnostic comparison. Using
\begin{equation}
v_{\rm EW}^{\rm PR}
=
\Lambda_{\rm EW}^{\rm PR}
...
\Lambda_{\rm EW}.
\end{equation}
They are used as diagnostic references. The weak-scale closure chain is
\begin{equation}
\boxed{
\left\{
q_{\rm bc},
...
The derivation of the electroweak scale is constrained by the same geometric rigidity that defines the compact fiber. The hierarchy possesses explicit failure modes that falsify the PR-II architecture if violated. First, the omission of the determinant term,
\begin{equation}
...
\Lambda_{\rm EW}
\end{equation}
as primary generation inputs violates the zero-new-parameter discipline of PR-II. The model remains viable only so long as these observables emerge as outputs of the hierarchy closure. With the weak-mixing and electroweak gauge-ledger established, we conclude the derivation of the scale. The subsequent section demonstrates how the compact non-Abelian phase projection recovers the full breadth of charged-current and neutral-current weak interactions, including beta-transition topology, \(Z\)-couplings, and the low-energy Fermi contact limit.

\section{Compact Order Mode and Tree-Level Scalar Coupling Ledger}
\label{sec:pr2_compact_order_mode_scalar_ledger}

Projection locking leaves one unbroken electromagnetic direction and three broken electroweak directions. This section parameterizes the associated order field, separates the compact orientation modes from the remaining scalar excitation, and derives the tree-level scalar couplings that follow from the order-field kinetic term. The previous section generated the weak order scale,
\begin{equation}
\boxed{
v_{\rm EW}^{\rm PR}
=
...
After isolating the single physical scalar order-mode $h_{\rm PR}$, we map its dynamics relative to the massive weak projection modes. By evaluating the compact order kinetic term in unitary gauge, we show that the same geometric constraints that generate $W$ and $Z$ masses also fix their tree-level scalar coupling ratios. The tree-level scalar gauge-boson couplings follow from the compact order kinetic term,
\begin{equation}
\mathcal L_{\rm order}
=
...
At low energy, the scalar coupling is determined by the dependence of the
effective fermion mass operator on the scalar order mode. In general,
\begin{equation}
g_{hff}^{\rm PR}
=
\left.
\frac{\partial m_f^{\rm PR}(h_{\rm PR})}
{\partial h_{\rm PR}}
\right|_{h_{\rm PR}=0}.
\end{equation}

For the tree-level matching adopted in this framework, the effective mass is taken to depend linearly on the order mode:
\begin{equation}
m_f^{\rm PR}(h_{\rm PR})
=
m_f^{\rm PR}
...
\right).
\end{equation}
Under this linear matching condition,
\begin{equation}
\boxed{
g_{hff}^{\rm PR}
=
...
\end{equation}
The linear dependence is a tree-level matching assumption; its derivation from the full compact overlap operator is assigned to the PR-III scalar precision future work. We find that the same overlap quantities that generate fermion mass candidates also determine the corresponding tree-level scalar coupling ledger. For the charged-lepton sector, this means
\begin{equation}
\boxed{
g_{hee}^{\rm PR}
=
...
With the scalar couplings for the $W$ and $Z$ bosons defined, we can trace these longitudinal modes back to the broken compact orientation of the parent fiber. Mapping these modes reveals how gauge mass terms and scalar interactions combine to maintain the tree-level cancellation needed for longitudinal unitarity. Specifically, these broken modes provide the directions for the longitudinal weak bosons:
\begin{equation}
\boxed{
\pi^a
\rightarrow
...
While the tree-level couplings between the scalar order-mode and the gauge ledger are fixed by the compact geometry, the physical mass of the scalar itself requires a dynamical evaluation of the vacuum curvature. It is imperative to clarify that PR-II does not claim a zero-new-parameter derivation of the scalar mass at this stage. Instead of inserting the observed scalar mass as an empirical fit, this framework formally defines the scalar mass as a geometric target, setting the stage for the PR-III precision program.

The scalar/order-mode coupling ledger is structurally recovered at tree level. However, the scalar mass itself is not inserted in as part of the calculation. In a compact-boundary formulation, the scalar mass must arise from the curvature of the order-locking action:
\begin{equation}
\boxed{
m_{h,{\rm PR}}^2
=
...
The preceding derivations collectively establish the foundational electroweak architecture, demonstrating that the gauge symmetries, mass matrices, physical currents, and interaction vertices emerge as geometric consequences of the compact non-Abelian phase projection.We also demarcate the strict boundaries of the current manuscript by explicitly cataloging the radiative, running, and higher-order precision tasks deferred to the subsequent PR-III program.
...
The scalar ledger is geometrically rigid and presents several immediate failure modes. First, the order field must intrinsically supply four real components before locking:
\begin{equation}
\Phi
\in
\mathbb C^2
...
Fourth, under the linear tree-level matching condition, the Dirac fermion scalar couplings must satisfy
\begin{equation}
g_{hff}^{\rm PR}
=
\frac{m_f^{\rm PR}}{v_{\rm EW}^{\rm PR}}.
\end{equation}
Failure of this relation would invalidate the adopted linear matching condition, while a nonlinear mass dependence would require evaluation through
\begin{equation}
g_{hff}^{\rm PR}
=
\left.
...
\begin{equation}
\boxed{
m_h\;\text{is not a generation input in this manuscript}.
}
\end{equation}
Such an insertion violates the zero-new-parameter discipline of PR-II. The scalar mass must be derived dynamically from the compact order-Hessian closure. Until that derivation is complete, it remains a PR-III precision target. With the compact order-mode and tree-level scalar coupling ledger established, we conclude this sector of the projection architecture. The subsequent section utilizes the compact non-Abelian current structure to recover the full breadth of charged-current and neutral-current weak interactions.

\section{Charged and Neutral Current Recovery}
\label{sec:pr2_charged_neutral_current_recovery}

 The weak interactions are the physical currents induced by state-motion along the $SU(2)_L$ fiber \cite{FeynmanGellMann1958}. We now demonstrate that the charged-current and neutral-current Lagrangians are the low-energy contact limits of the parent projection geometry. The central chain is defined as
\begin{equation}
\boxed{
SU(2)_L
\rightarrow
...
The transition from static geometry to active weak processes begins with the definition of the topological ladder. The "charged-current" interactions such as the decay of a neutron or the transition of a quark—are not governed by extrinsic force rules. Rather, they are the intrinsic response of the $SU(2)_L$ fiber as a state undergoes a discrete geometric shift. We formalize these transitions as simple "ladder moves" within the internal manifold, demonstrating that the standard beta-decay topology is the mechanical result of state-motion across the compact-boundary projection. The charged-current ladder operators are
\begin{equation}
T^\pm
=
T_1\pm iT_2.
...
\end{equation}
Thus the low-energy Fermi interaction is the contact limit of the massive charged projection modes. Using the weak-scale candidate from Section~\ref{sec:pr2_generated_weak_scale_fermi_constant},
\begin{equation}
\boxed{
v_{\rm EW}^{\rm PR}
=
...
While the charge-current ladder handles identity shifts, the neutral sector describes the state response orthogonal to the electromagnetic boundary \cite{Glashow1961}. The physical photon and the $Z$ boson are not independent entities but are the resulting eigenstates of a single geometric rotation within the parent fiber. By formalizing the neutral-current projection, we demonstrate that the fermion coupling assignments emerge as the trigonometric consequences of the manifold's orientation. The $Z$-mediated interaction remains locked to the same compact spectral scale as the charged sector, ensuring that the entire neutral ledger is a consistent hardware output of the projection-locking mechanism. The neutral modes are
\begin{equation}
A_\mu
=
\sin\theta_W\,W_\mu^3
...
\end{equation}
At low momentum transfer, $q^2\ll M_Z^2$, integrating out the massive neutral projection mode gives the neutral-current contact coefficient
\begin{equation}
\boxed{
\frac{g_Z^2}{8M_Z^2}
=
...
The final structural check of the gauge sector involves the mass values of the $W$ and $Z$ bosons. While the ratios and symmetries of these masses are now geometrically locked, their specific numerical values are scale-dependent, governed by the running of the electromagnetic coupling from the PR-I low-energy boundary ($\alpha \approx 1/137$) to the weak scale ($\alpha(M_Z) \approx 1/128$). A diagnostic tree-level comparison is performed using standard running-coupling values to show that the framework naturally lands within the physical regime. This sets the stage for the final "Running Layer" of the theory, where the transition from the macroscopic boundary to the microscopic scale is derived rather than matched. The PR-II mass geometry is closed by
\begin{equation}
M_W
=
M_Z\cos\theta_W,
...
4\pi\alpha(M_Z).
\end{equation}
PR-I fixed the low-energy electromagnetic boundary, $\alpha_{\rm PR,bc}^{-1}=137.036361812007$, but the running map $\alpha_{\rm PR}(0)\rightarrow\alpha_{\rm PR}(M_Z)$ is a radiative matching layer. Therefore, precision \(W/Z\) mass comparison is not claimed in this section. As a diagnostic tree-level branch, using a weak-scale electromagnetic coupling near
\begin{equation}
\boxed{
\alpha(M_Z)^{-1}=127.955
}
...
They are massiveprojection excitation modes of the broken electroweak fiber. 
...
The recovery of the physical weak currents is a deterministic output of state-motion along the gauge fiber. The architecture is falsified if it cannot preserve the following contact limits and charge topologies. 
...
Q = T_3 + \frac{Y}{2},
\end{equation}
admits the minimal matter ledger required to populate the electroweak and color sectors. This analysis is structural, defining the internal configuration of a single generation. While the derivation of the three-generation count is addressed through the compact flavor-sheet geometry in Section~\ref{sec:pr2_generation_origin_flavor_sheets}, the present section establishes the anomaly-free criteria that any projected generation must satisfy to maintain the manifold's integrity.
...
To verify the internal consistency of the matter sector, we map the specific charge residues for individual fermion states following the projection locking step. The electric charge is not a fundamental label, but the specific geometric coordinate a state occupies on the residual electromagnetic boundary. By applying the locking relation $Q = T_3 + Y/2$ to the chiral Weyl multiplets, we demonstrate that the observed fractional and integer charges of the Standard Model are the deterministic outputs of the manifold's orientation. We show that while the parent weak representation is chiral, the resulting electromagnetic projection is vector-like, providing the necessary symmetry for the stability of the long-range force sector. The residual electromagnetic charges follow the residual electromagnetic generator~\ref{equ:residual_charge} For the left-handed quark doublet,
\begin{equation}
Q_L=
\begin{pmatrix}
u_L\\
...
We check the internal electroweak consistency by the interference between the non-Abelian weak fiber and the hypercharge phase. The mixed weak–hypercharge anomaly, $[SU(2)_L]^2U(1)_Y$, represents the balancing act between the baryonic and leptonic sectors of the matter ledger. We audit the contribution of the weak doublets, demonstrating that the quark and lepton multiplicities are calibrated to cancel the anomaly coefficient. This result confirms that the manifold’s hardware is grounded, such that the electroweak currents can propagate across the fiber without mathematical divergence. Where only weak doublets contribute and the one-generation coefficient is
\begin{equation}
\boxed{
[SU(2)_L]^2U(1)_Y:
\qquad
...
Global compatibility requires the alignment between the microscopic matter ledger and the macroscopic gravitational manifold established in PR-I. The mixed gravitational–hypercharge anomaly, $[\mathrm{grav}]^2U(1)_Y$, identifies whether the identity-labels of the fermion states induce a global "shear" on the background curvature. This cancellation ensures that the discrete microscopic charges do not destabilize the smooth gravitational response of the projection. We now audit the total hypercharge sum across all projected states, demonstrating that the ledger is balanced against the gravitational foundation inherited from the macroscopic theory. It is proportional to the sum of hypercharges over all left-handed Weyl fields, including multiplicities. 
For one generation,
\begin{equation}
\boxed{
[\mathrm{grav}]^2U(1)_Y:
...
By applying the individual audits of the color, weak, and hypercharge sectors, we now synthesize these findings into a unified, stable blueprint for the matter sector. The Anomaly-Free One-Generation Ledger, representing the set of fermion coordinates required to populate the non-Abelian manifold without inducing mathematical divergence or "current leakage." This specific representation is not merely an empirical list of particles, but the solution compatible with the $SU(2)_L \times U(1)_Y \to U(1)_{em}$ locking geometry. By satisfying both the local gauge sums and the global Witten condition, this ledger establishes the "Locked-In" generation block upon which the multi-generational architecture of Section~\ref{sec:pr2_generation_origin_flavor_sheets} is constructed. Combining the local and global checks, the one-generation ledger
\begin{equation}
\mathcal R_{\rm EW}^{(1)}
=
(3,2)_{1/3}
...
If the electroweak projection cannot intrinsically carry the specific chiral assignments established below, the entire microscopic extension is mathematically rejected.
...
\neq0.
\end{equation}
So that the inclusion of the lepton doublet is a structural requirement to exactly cancel the quark contribution.

Third, the lepton doublet hypercharge is constrained. If it deviates from the exact assignment
\begin{equation}
Y_L^\ell=-1,
\end{equation}
...
Now with the fermion representation and anomaly-cancellation established, the subsequent section utilizes the compact projection-locking geometry to derive the three-generation ledger and the corresponding generation-sheet flavor hierarchy.
...
1.
\end{equation}
The broken projection space therefore has dimension
\begin{equation}
\dim
\left[
\frac{
...
\end{semilogyaxis}
\end{tikzpicture}
\caption{Three compact generation sheets arise from the broken electroweak projection quotient. The terminal matter--displacement overlaps are hierarchically ordered by the determinant ratio $r_G=c_{\rm bc}/q_{\rm bc}$.}
...
The compact electroweak quotient has three broken directions, 

\begin{equation}
\dim \left[\frac{SU(2)_L\times U(1)_Y}{U(1)_{\rm em}} \right] = 3. 
\end{equation}

PR-II proposes identifying these directions with three fermion-generation sheets. This geometric identification supplies the value $N_{\rm gen}^{\rm PR}=3$, while anomaly cancellation confirms that
three replicated chiral ledgers remain consistent. The identification is a closure candidate; it does not follow from coset dimension or anomaly cancellation alone. Given this three-sheet assignment, the compact determinant ratio $r_G=c_{\rm bc}/q_{\rm bc}$ is used to define an ordered overlap hierarchy. The hierarchy is generated by the determinant suppression ratio inherited from PR-I:
\begin{equation}
\boxed{
r_G
=
...
For a fermion sector \(f\), define the charge norm
\begin{equation}
\mathcal C_f
=
C_2^{SU(3)}(f)
...
\end{equation}
 The generation-sheet hierarchy is universal, while the overall sector normalization is shifted by compact representation data. These spectra are not yet mass formulas. They are compact flavor-overlap ledgers. Physical fermion masses require sector-specific scale rules, running, and projection matching, developed in later sections.

 \begin{table}[ht!]
\centering
\caption{Sector-specific overlap spectra. These are structural overlap ledgers, not physical fermion mass predictions.}
...
To maintain the zero-new-parameter discipline of the PR-II architecture, it is essential to categorize the claims made in the preceding flavor and generation derivations. Not all derived components carry the same theoretical weight; some represent structural boundaries, while others serve as intermediate geometric ledgers that await further kinematic and running refinement. We clarify the precise interpretative status of the anomaly consistency, the three-generation counting rule, and the overlap spectra. By delineating these statuses, we establish a boundary between the foundational flavor geometry achieved here and the precision scaling obligations deferred to subsequent sections. The results of this section have three distinct statuses.
...
Deriving three generations from the broken electroweak quotient introduces severe topological constraints. Any attempt to modify the generation-sheet hierarchy manually triggers the following projection errors.
...
\frac{q_{\rm bc}}{c_{\rm bc}},
\end{equation}
the generation hierarchy becomes non-physically inverted, structurally collapsing the overlap ledger.
...
U_{\rm PMNS}^{\dagger}U_{\rm PMNS}\neq I_3
\end{equation}
would falsify the framework by failing to preserve the universal neutral-current interaction structure.
...
The falsification boundary is therefore. PR-II must structurally derive the generation count from the compact electroweak quotient, preserve anomaly cancellation under replication, generate an ordered overlap hierarchy from \(r_G=c_{\rm bc}/q_{\rm bc}\), and enforce exclusively unitary flavor rotations. Failure at any of these checkpoints fundamentally breaks the compact flavor-sheet program.

With the generation-origin and compact flavor-sheet geometry established, we conclude this sector. The subsequent section applies this three-sheet flavor architecture to the charged-lepton sector, deriving the first zero-new-parameter precision mass candidate.

\section{Charged-Lepton Precision Candidate}
\label{sec:pr2_charged_lepton_precision_candidate}

The compact flavor-sheet produced three hierarchical matter displacement overlaps and sector-specific overlap ledgers. 
...
The charged-lepton sector is represented on a three-sheet square-root mass plane. The proposed associates the two charged-current ladder orientations with the nine ordered sheet-pair incidences of the three-generation ledger. This motivates the candidate phase
\begin{equation}
\theta_K^{\rm PR}
=
\frac{N_{\rm charged\ ladder}}{N_{\rm pair}}
=
\frac{2}{N_{\rm gen}^2}
=
\frac29.
\end{equation}
The ratio \(2/N_{\rm gen}^2\) is adopted here as a phase in radians. Its uniqueness remains a separate flavor-boundary derivation obligation. Once the three-sheet trigonometric parameterization is adopted, the Koide ratio follows algebraically for any value of the phase. The PR-II content therefore lies in the proposed phase, the sheet assignment, and the compact scale rather than in the \(2/3\) identity alone. We define the charged-lepton sheet factors
\begin{equation}
k_a(\theta_K)
=
\left[
...
The empirical Koide relation \cite{Koide1983,Koide1983Erratum} is reproduced algebraically by the three-sheet square-root parameterization. The trigonometric identities below remove the phase dependence from the Koide ratio and leave the overall scale \(M_\ell\) undetermined. The model-specific content therefore lies in the proposed sheet phase, the sheet assignment, and the independent compact-boundary scale rule. We let
\begin{equation}
m_a
=
M_\ell k_a(\theta_K).
...
\end{equation}
The trigonometric parameterization enforces the Koide ratio \(2/3\) for any value of \(\theta_K\). The model-specific content of PR-II is therefore not the ratio alone, but the proposed phase \(\theta_K^{\rm PR}=2/9\), the sheet assignment, and the compact scale \(M_\ell^{\rm PR}\). We note that the parameterization does not determine the overall
mass scale by itself.

\subsection{Charged-Lepton Compact Scale}
\label{sec:pr2_charged_lepton_compact_scale}
We define the following compact-boundary candidate for the charged-lepton scale. Its numerical value is generated from the inherited boundary ledger without inserting the observed charged-lepton masses. The uniqueness of this scale map remains a separate derivation obligation. The suppression ratio is:
\begin{equation}
r_G
=
\frac{c_{\rm bc}}{q_{\rm bc}}
...
\end{equation}
The second scale correction is the generation determinant leakage averaged over the residual compact boundary,
\begin{equation}
\boxed{
C_2
=
...
By applying the dimensionless flavor phase, \(\theta_K^{\rm PR}\), and the dimensionful scale, \(M_\ell^{\rm PR}\), now fixed by the compact electroweak boundary, the architecture achieves full generative closure for the charged-lepton sector. We show that these synthesizes as geometric components to output the explicit masses of the electron, muon, and tau. This evaluation proceeds entirely without importing empirical mass data. The resulting theoretical spectrum is subsequently compared with the observed pole masses \cite{PDG2024} solely as a diagnostic test, demonstrating that the compact projection recovers the physical charged-lepton hierarchy with precision exceeding 30 parts per million. So we begin by using
\begin{equation}
\boxed{
\theta_K^{\rm PR}=\frac29,
}
...
The charged-lepton spectrum can now be compared with the structural overlap ledger introduced earlier. These two constructions describe different aspects of the same sector. The overlap matrix sets the ordering and broad hierarchy inherited from the compact boundary, but it is not itself a precision mass formula. The Koide-sheet construction instead fixes the relative distribution of the square-root mass vector across the three generation sheets. Because both depend on the same generation count, \(N_{\rm gen}=3\), they form a single closure chain: the compact boundary determines the sector scale and hierarchy, while the sheet phase determines the internal mass pattern. The sector-specific charged-lepton overlap spectrum is therefore
\begin{equation}
\boxed{
D_\ell
=
...
D_\ell
\quad
\text{sets the sectoral overlap structure,}
}
\end{equation}
while
\begin{equation}
...
The derivation of the charged-lepton mass spectrum adheres to the zero-new-parameter discipline. The generative inputs are restricted entirely to the pre-established compact boundary and geometric counting rules of compact scale. Empirical lepton masses, mass ratios, fitted flavor angles, and arbitrary Yukawa couplings are explicitly prohibited from entering the generative chain. Because it is derived from a single boundary scale and a three-sheet topology rather than a phenomenological fit, this spectrum is formally designated as a:
\begin{equation}
\boxed{
...
The charged-lepton candidate represents a highly deterministic precision test. Because the square-root mass vector is fixed by the flavor-plane topology, any deviation in the following geometric rules breaks the mass generation.
...
\end{equation}
fails to reproduce the precision candidate. The PR-II topology mandates the charged-ladder incidence density:
\begin{equation}
...
The boundary strong-coupling candidate emerges directly from the primary geometric boundary components, defined as the product of the boundary coordinates offset by the residual compact deficit:
\begin{equation}
\left(
\alpha_3^{\rm PR}
\right)^{-1}
...
(1-c_{\rm bc}).
\end{equation}
Inserting the established structural values \(q_{\rm bc} = 10.905007182855176\) and \(c_{\rm bc} = 0.796684464847899\), we immediately obtain the numerical evaluation of the stated closure formula:
\begin{equation}
\boxed{
\alpha_3^{\rm PR}
=
...
For the running diagnostic developed below, $\alpha_3^{\rm PR}$ is interpreted as an $\overline{\rm MS}$ type coupling anchor at the neutral matching surface. This scale-and-scheme assignment is a matching convention rather than an independently derived identity of the compact-boundary formula.

This evaluation relies exclusively on the pre-established compact-boundary data. Empirical parameters such as \(\alpha_s(M_Z)\) \cite{GrossWilczek1973}, \(\Lambda_{\rm QCD}\), quark masses, CKM angles, or weak-scale observables are explicitly prohibited from entering the generation formula; they may only serve as post-calculation diagnostic benchmarks within a separately derived running or matching layer. The causal chain for the strong-coupling candidate is therefore exclusively geometric:
\begin{equation}
\boxed{
\left\{
q_{\rm bc},\,c_{\rm bc}
...
The PR-II architecture derives these heavy-quark thresholds purely from the established compact-boundary geometry. Utilizing the determinant suppression ratio (\(r_G\)), the weak boundary deficit (\(\Delta_{\rm EW}\)), the compact leakage factors (\(C_1, C_2\)), and the terminal weak displacement anchor (\(\Lambda_{\rm EW}^{\rm PR}\)) as defined in previous sections. The top-sector threshold is identified with the terminal weak displacement anchor, modulated by the compact leakage factors:
\begin{equation}
\mu_t^{\rm PR}
=
...
The bottom-sector threshold is generated by a single color-averaged determinant step, refined by the down-type hypercharge-boundary loss:
\begin{equation}
\mu_b^{\rm PR}
=
\Lambda_{\rm EW}^{\rm PR}
...
The charm-sector threshold requires a new geometric projection: the singlet-plus-adjoint compact factor, defined as \(A_{\rm EW} = (1+3c_{\rm bc})/2\). The threshold is then generated by two determinant suppressions, the color Casimir lift (\(4/3\)), and this adjoint boundary gain:
\begin{equation}
\boxed{
\mu_c^{\rm PR}
=
...
\subsection{Three-Loop Coupling Running with Leading Threshold Matching}
\label{sec:pr2_qcd_threshold_ledger}

The coupling evolution is evaluated using the displayed three-loop beta-function coefficients. Flavor thresholds are matched at leading
continuity order,
\begin{equation}
\alpha_s^{(n_f)}(\mu_{\rm th})
=
\alpha_s^{(n_f-1)}(\mu_{\rm th}),
\end{equation}
while the quoted mass-running diagnostics use the leading anomalous-dimension exponent. The resulting calculation is therefore a three-loop coupling-running ledger with leading threshold matching, not a uniform three-loop quark-mass determination. Higher-order decoupling coefficients and higher-loop mass anomalous dimensions are deferred to the PR-III QCD precision layer future research.
\begin{equation}
\boxed{
...
The numerical evaluation of the resulting coupling-running and leading-matching ledger is consolidated in Table~\ref{tab:pr2_qcd_lambda_scales}.
...
It is important to maintain structural clarity regarding the status of these outputs. The quantities in Table~\ref{tab:pr2_qcd_running_diagnostics} are purely running diagnostics, not independent pole-mass predictions. Furthermore, while the compact strong-coupling value, \(\alpha_3^{\rm PR}\), is generated entirely from PR boundary data, the neutral electroweak matching scale utilized in this numerical QCD ledger inherits the formal electroweak running-layer status discussed in previous sections. The full electroweak precision matching of this neutral scale is therefore formally deferred to the radiative/running obligations of the PR-III precision program.

\subsection{Light-Sector Compact Confinement Average}
\label{sec:pr2_light_sector_confinement_average_main}

The light-quark sector is evaluated through a confined, low-energy projection window in which individual colored asymptotic states are not resolved. The unresolved compact boundary is therefore characterized by
two normalized endpoint response weights. Let the normalized endpoint response weights be
\begin{equation}
\boxed{
w_{\rm singlet}=1,
\qquad
...
\end{equation}
The unresolved compact confinement response is defined by
\begin{equation}
\boxed{
C_{\rm conf}
=
...
\end{equation}
and recovers both endpoint weights algebraically:
\begin{equation}
\boxed{
...
\end{equation}
The quantity \(C_{\rm conf}\) is therefore not fitted to the light-quark masses; it is the endpoint average defined above.
...
The reference values are not generation inputs; they are listed only to show the post-generation diagnostic residuals. The light-quark entries should be read as low-energy running-mass diagnostics. The heavy-quark entries should be read as compact threshold-scale candidates.
...
The generative inputs are restricted entirely to the pre-established compact boundary parameters, the terminal weak anchor, and the compact confinement response (\(C_{\rm conf} = \frac{1+c_{\rm bc}}{2}\)):
\begin{equation}
\boxed{
\left\{
q_{\rm bc},\,
...
Utilizing the generated strong-coupling anchor, the heavy-quark thresholds, and the light-quark hierarchy from the compact-boundary ledger, it is important to establish the mathematical boundaries that make this derivation falsifiable. We note that the displayed quark-sector formulas do not use observed quark masses, \(\alpha_s(M_Z)\), or \(\Lambda_{\rm QCD}\) as fitted inputs. They define a compact-boundary closure candidate whose physical interpretation still requires a specified QCD scheme, running prescription, and threshold matching. The uniqueness of the proposed functional forms remains a separate derivation obligation. The derivation of the quark sector is constrained by the same geometric rigidity that defines the parent non-Abelian fiber and possesses the following failure modes:

First, misapplying the determinant suppression ratio as its inverse fundamentally inverts the physical hierarchy. Utilizing
\begin{equation}
\frac{q_{\rm bc}}{c_{\rm bc}}
\end{equation}
as a scaling operator fails the structural projection and is mathematically rejected.

Second, naively treating the dimensionless overlap operators, \(D_u\) and \(D_d\), as direct GeV-valued mass parameters fundamentally violates the dimensional status of the projection. These ledgers define the dimensionless compact-overlap geometry, not the dimensionful mass scales.
...
Fourth, ignoring the light-sector confinement average,
\begin{equation}
C_{\rm conf}
=
\frac{1+c_{\rm bc}}{2},
...
This section constructs boundary candidates for both mixing ledgers. 
...
Instead, the topological constraints of both the CKM and PMNS architectures are generated exclusively from the pre-established compact-boundary constants as the determinant suppression ratio (\(r_G\)), the weak boundary deficit (\(\Delta_{\rm EW}\)), the compact confinement response (\(C_{\rm conf}\)), the adjoint boundary average (\(A_{\rm EW}\)), and the geometric generation count (\(N_{\rm gen}^{\rm PR}=3\)). The generative boundary for the entire flavor-mixing sector is therefore confined to the following structural set:
\begin{equation}
\boxed{
\left\{
r_G,\,
...
PR-II represents the CKM matrix as the relative left-handed rotation between the up-type and down-type flavor bases. The present construction does not yet derive those rotations by diagonalizing explicit PR mass matrices. Instead, it proposes compact-boundary rules for the mixing angles and phase and evaluates the resulting matrix in the standard unitary parameterization \cite{Cabibbo1963,ChauKeung1984}. The defining relation is
\begin{equation}
\boxed{
V_{\rm CKM}
=
...
The closure candidate uses the following parameters constructed from the shared boundary ledger. The proposed Cabibbo sheet-angle rule is
\begin{equation}
\boxed{
\lambda_{\rm PR}
=
...
The CKM hierarchy coefficient isolates the residual boundary proportion:
\begin{equation}
\boxed{
A_{\rm CKM}^{\rm PR}
=
...
The compact CKM phase candidate is defined by the three-sheet phase offset and the weak boundary deficit:
\begin{equation}
\boxed{
\delta_{\rm CKM}^{\rm PR}
=
...
\end{equation}
Because the standard unitary CKM parameterization is used, unitarity is exact by construction. The reported residual therefore tests the numerical implementation:
\begin{equation}
\boxed{
\max
\left|
...
\end{equation}
and the corresponding Jarlskog invariant \cite{Jarlskog1985}:
\begin{equation}
\boxed{
J_{\rm CKM}^{\rm PR}
=
...
The nonzero Jarlskog invariant follows from the proposed compact-boundary mixing-angle and phase rules. It is therefore a nontrivial output of those rules, but it is not yet an independent consequence of the structural overlap matrices. A full derivation would require explicit up- and down-sector mass matrices, their left-handed diagonalizing rotations, and the relation $V_{\rm CKM}=U_{u,L}^{\dagger}U_{d,L}$. The current result is accordingly classified as a CKM closure candidate. The generative parameters and their structural status are consolidated in Table~\ref{tab:pr2_ckm_candidate_main}.
...
This result is interpreted as a CKM mixing hierarchy candidate. Observed CKM elements, quark masses, and the measured CP phase are not used as generation inputs; they enter only as diagnostic references. The proposed compact-boundary angle and phase rules yield a unitary matrix with hierarchical off-diagonal elements and nonzero CP violation. Deriving the rotation from explicit PR quark mass matrices remains an open flavor-closure task.
...
The PMNS atmospheric-angle candidate is generated by distributing the singlet-plus-adjoint compact factor over the three generation sheets:
\begin{equation}
...
\end{equation}
As in the CKM construction, unitarity follows from the standard unitary PMNS parameterization. The numerical implementation gives
\begin{equation}
\boxed{
\max
\left|
...
This result is interpreted as a PMNS lepton-mixing structure candidate. The proposed compact-boundary rules yield large solar and atmospheric mixing, a smaller reactor angle, and a nonzero leptonic CP phase. Because the standard unitary parameterization is used, the reported unitarity residual tests numerical implementation rather than the uniqueness of the angle rules. Explicit lepton and neutrino mass operators remain necessary for a derivation of the mixing matrix. The neutrino mass-spectrum and Majorana candidates are addressed separately in Section~\ref{sec:pr2_neutrino_mass_majorana_boundary}.

\subsection{CKM--PMNS Contrast}
\label{sec:pr2_ckm_pmns_contrast}
The proposed quark and lepton mixing rules produce qualitatively different patterns. The CKM candidate is determinant-suppressed and hierarchical, whereas the PMNS candidate is built from compact boundary averages and the weak boundary deficit. The CKM hierarchy is
\begin{equation}
|V_{us}|>|V_{cb}|>|V_{ub}|,
\end{equation}
...
\end{equation}
We do not impose one universal flavor-angle pattern across quarks and leptons. The quark sector is compact-determinant-suppressed, while the lepton sector is compact-average-dominated. This distinction is consistent with the observed qualitative pattern that CKM mixing is small and hierarchical,
whereas PMNS mixing is large with a small but nonzero reactor angle.

\subsection{Input Discipline and Status}
\label{sec:pr2_ckm_pmns_input_discipline_status}

The CKM and PMNS candidates are generated exclusively from the pre-established compact geometric constants: the primary boundary coordinates (\(q_{\rm bc}, c_{\rm bc}\)), the determinant suppression ratio (\(r_G\)), the weak boundary deficit (\(\Delta_{\rm EW}\)), the confinement response and singlet-plus-adjoint factor (\(C_{\rm conf}, A_{\rm EW}\)), and the geometric generation count (\(N_{\rm gen}^{\rm PR}\)). We do not use
\begin{equation}
V_{\rm CKM}^{\rm obs},
\qquad
U_{\rm PMNS}^{\rm obs},
...
The proposed CKM and PMNS closure rules have several clear failure modes. First, substituting the raw determinant ratio for the Cabibbo angle does not reproduce the stated CKM candidate. Using
\begin{equation}
\lambda \neq r_G
\end{equation}
fails the structural evaluation. The present closure candidate instead uses
\begin{equation}
\lambda_{\rm PR}
=
\sqrt{
...
Second, setting the CKM phase to zero gives \(J_{\rm CKM}=0\) and therefore does not reproduce the nonzero-CP candidate developed above. This tests the proposed phase rule; it does not independently prove that the rule is unique.
...
Fourth, the displayed absolute-value matrices are diagnostic summaries rather than complete mixing matrices. The underlying complex matrices are constructed with the standard unitary parameterizations and therefore satisfy
\begin{equation}
V_{\rm CKM}^\dagger V_{\rm CKM}=I_3,
\qquad
U_{\rm PMNS}^\dagger U_{\rm PMNS}=I_3.
...
\delta_{\rm CP}^{\rm obs},
\end{equation}
as primary generation inputs violates the zero-new-parameter discipline of PR-II. The framework remains viable only so long as these observables emerge as outputs of the topological closure, permitted only as post-calculation diagnostic benchmarks. If empirical parameter fitting is required to achieve mixing closure, the architecture is falsified. Our next objective is to extend the compact-boundary flavor structure into the neutrino mass sector, applying the same geometric principles to define the neutrino mass hierarchy and the associated Majorana phase boundary.
...
The neutrino mass candidate begins with a residual singlet sheet $m_1^{\rm PR}=0$. This identifies the lightest neutrino with the neutral residual compact-singlet projection. The heavier two neutrino states are generated by determinant lifting on top of a Planck-suppressed weak-order scale. The determinant ratio is
\begin{equation}
r_G
=
\frac{c_{\rm bc}}{q_{\rm bc}}.
...
\end{equation}
The factor \(v_{\rm EW}^2/\bar M_{\rm Pl}\) is the natural weak-order, Planck-suppressed scale. The determinant lift \(r_G^{-3}\) reflects the three-sheet generation ledger. The solar-to-atmospheric splitting ratio is generated by the weak boundary deficit projected through the residual compact boundary:
\begin{equation}
\boxed{
R_{21}^{\rm PR}
=
...
PR-II adopts the residual compact-singlet sheet as a massless anchor for the neutrino closure candidate:
\begin{equation}
m_1^{\rm PR} = 0.
\end{equation}

Together with the proposed determinant-lift and solar-to-atmospheric ratio rules, this boundary condition produces a normal-ordering mass-spectrum candidate:
\begin{equation}
...
\end{equation}
Given this candidate spectrum, the associated mass-squared splittings are \cite{EstebanNuFIT2024}:
\begin{equation}
\boxed{
\Delta m_{21}^{2,{\rm PR}}
=
...
\end{equation}
The corresponding candidate mass-splitting ratio is
\begin{equation}
\boxed{
\frac{
\Delta m_{21}^{2,{\rm PR}}
...
The same candidate spectrum gives corresponding values for the mass sum and beta-decay effective mass. The total cosmological neutrino mass \cite{Planck2018Cosmology} sum is
\begin{equation}
\boxed{
\sum_i m_i^{\rm PR}
=
...
The values reported in Table~\ref{tab:pr2_neutrino_mass_candidate_main} are outputs generated by the stated neutrino closure rules. Experimentally observed neutrino splittings, tritium beta-decay limits, and cosmological mass constraints are prohibited from acting as inputs; they are permitted to enter the framework only post-generation as diagnostic benchmarks.

\subsection{Majorana-Dominant Compact-Singlet Projection}
\label{sec:pr2_majorana_dominant_compact_singlet_projection}
The generated neutrino scale has the form of a dimension-five Majorana-like operator \cite{Weinberg1979}. Within the present compact-overlap construction, a purely Dirac interpretation would require an effective coupling of order \(10^{-13}\), which is not generated by the overlap rule developed here. The present construction therefore favors a Majorana-dominant channel, while not excluding Dirac or mixed mechanisms outside the stated ansatz. The relevant scale is
\begin{equation}
\boxed{
\frac{
(v_{\rm EW}^{\rm PR})^2
...
\end{equation}
This small number is not produced by the overlap hierarchy. 
...
Given the PR-II neutrino mass-spectrum candidate, PMNS mixing candidate, and compact Majorana phase rule, the corresponding neutrinoless double-beta effective mass can be evaluated. The numerical result is exact with respect to those stated inputs; the phase relation
\begin{equation}
\phi_M^{\rm PR}
=
\pi C_{\rm conf}
\end{equation}
remains a compact-boundary closure candidate rather than an independently derived theorem. For the massless anchor ($m_1^{\rm PR}=0$), the neutrinoless double-beta effective mass \cite{Majorana1937, SchechterValle1982} is governed entirely by the interference of the two remaining states:
\begin{equation}
\boxed{
m_{\beta\beta}
=
...
Rather than treating the relative phase shift as a fitted phenomenological unknown, the architecture proposes the compact-boundary relative Majorana interference phase through the confinement response:
\begin{equation}
\boxed{
\phi_M^{\rm PR}
=
...
Incorporating the previously generated CP-violating phase ($\delta_{\rm CP}$), this phase rule gives the following higher-generation Majorana phase:
\begin{equation}
\boxed{
\alpha_{31}^{\rm PR}
=
...
Evaluating the effective mass with these candidate phases gives:
\begin{equation}
\boxed{
m_{\beta\beta}^{\rm PR}
=
...
For this normal-ordering mass-spectrum candidate, arbitrary Majorana \cite{MakiNakagawaSakata1962} phases would permit the envelope
$1.410\ {\rm meV}\leq m_{\beta\beta} \leq 3.642\ {\rm meV}$. The proposed compact-boundary phase selects $m_{\beta\beta}^{\rm PR} = 1.508\ {\rm meV}$, near the lower-interference side of that envelope. The single value is therefore conditional on the phase rule $\phi_M^{\rm PR}=\pi C_{\rm conf}$.

\subsection{Flavor-Basis Majorana Mass Matrix}
\label{sec:pr2_flavor_basis_majorana_mass_matrix}
Given the neutrino mass-spectrum candidate, PMNS mixing candidate, and Majorana phase rule, the corresponding flavor-basis Majorana mass matrix is
\begin{equation}
\boxed{
M_\nu^{\rm PR}
=
...
By construction, this matrix satisfies the Majorana symmetry condition:
\begin{equation}
\boxed{
\left(
M_\nu^{\rm PR}
...
Because the spectrum is generated from the stated boundary rules rather than empirically fitted, the formal structural status of this section is explicitly designated as a:
\begin{equation}
...
The derivation of the neutrino mass spectrum provides an important boundary for falsification within the framework. Because it operates across a dimension-five topological gap, any structural tampering instantly breaks the predictive closure.
...
Second, artificially reversing the determinant hierarchy projection—substituting a geometric suppression ($r_G^{+3}$) in place of the topological lift ($r_G^{-3}$) catastrophically fails to generate the heavy atmospheric mass scale ($m_3^{\rm PR}$).
...
Using only the established compact-boundary constants, PR-II must produce a normal-ordered neutrino spectrum, the observed hierarchy of mass splittings, a cosmologically viable total mass, and a definite Majorana interference phase. If matching the data requires fitting any step in this chain, the result no longer qualifies as a zero-new-parameter closure and must instead be treated as a phenomenological model.

To complete the manuscript, we present the assembled final, comprehensive audit map. It explicitly separates the structural identities from the diagnostic ledgers, formally declaring the no-free-parameter status and the ultimate falsification boundaries of the PR-II manuscript.
...
Target observables are not used as inputs for generation, only as diagnostic references.
...
% These two commands create a 8pt forcefield above and below every line
...
\)
&
Tree-level \(hWW\) and \(hZZ\) couplings, together with an \(hff\) coupling conditional on linear order-mode matching.
&
...
The repository is not a substitute for formal derivation. It documents the stated inputs, reproduces the displayed arithmetic, and checks the implementation for target-input leakage; it does not establish the uniqueness of the proposed closure formulas.

\subsection{Remaining Derivation Obligations}
\label{sec:pr2_remaining_derivation_obligations}

The remaining obligations concern formal derivation, uniqueness, and precision matching. They define the work required to test whether the present closure candidates follow uniquely from the compact-boundary architecture or must instead remain independent ansätze. The principal objectives for the next derivation layer are:

\begin{enumerate}
    \item \textbf{Electroweak Boundary Uniqueness:} Determine whether the compact boundary-deficit rule is unique under the stated assumptions and characterize any admissible alternatives.
    ...
    \item \textbf{Generation Topology:} Formalize the precise topological mechanism that defines the projection-sheet origin of the three-generation chiral ledger (\(N_{\rm gen}^{\rm PR}=3\)).
    \item \textbf{Charged-Lepton Phase Rigidity:} Derive the proposed Koide sheet phase from an explicit flavor-boundary operator, or retain it as an independent closure ansatz.
    \item \textbf{QCD Precision Completion:} Finalize the QCD renormalization scheme, running maps, confinement conventions, and threshold-matching prescriptions required to compare the generated compact quark thresholds and light-quark candidates with scheme-specified running and threshold mass definitions.
    \item \textbf{Flavor Mixing Uniqueness:} Derive CKM and PMNS mixing from explicit PR mass operators and their diagonalizing rotations, formally closing the unitary mixing architecture.
    \item \textbf{Majorana Phase Theorem:} Derive or falsify the proposed Majorana phase rule from an explicit neutrino-sector operator governing neutrinoless double-beta decay.
\end{enumerate}

These tasks define the PR-III precision and derivation program. Their outcome will determine which PR-II closure rules can be derived from explicit operators or actions, which require modification, and which should remain phenomenological or structural ansätze.

\section{Conclusion}
\label{sec:pr2_conclusion}

Projection Relativity II (PR-II) extends the PR-I compact-boundary architecture into the tree-level electroweak and flavor sectors. The aim is not to replace the Standard Model, but to test whether several quantities normally introduced independently can be organized as outputs of a shared projection framework. The manuscript therefore separates standard structural identities from proposed closure rules, precision candidates, and diagnostic comparisons.

Within this framework, electroweak locking, mass-scale candidates, and flavor constructions are expressed using a common compact-boundary ledger. The manuscript develops the following structural results and closure candidates:
\begin{itemize}
    \item \textbf{The Electroweak Architecture:} Projection locking recovers the tree-level photon and \(W/Z\) mass relations, while the weak-mixing and Fermi-scale formulas are presented as compact-boundary closure candidates.
    \item \textbf{The Generation Topology:} A three-generation projection identification is proposed from the three broken electroweak directions, while anomaly cancellation confirms the consistency of the replicated chiral ledger.
    \item \textbf{The Mass and Mixing Ledger:} The stated compact-boundary formulas generate charged-lepton and quark hierarchy candidates together with unitary CKM and PMNS mixing candidates. Derivation from explicit flavor mass operators remains an open task.
    \item \textbf{The Neutrino Sector:} The proposed Planck-suppressed closure rules yield a normal-ordering, Majorana-dominant mass-spectrum candidate and a conditional neutrinoless double-beta effective-mass candidate.
\end{itemize}

The central methodological constraint is the separation of generation inputs from diagnostic reference values. The displayed candidates are evaluated from the stated compact-boundary rules without inserting the target observables as fitted numerical inputs:
\begin{equation}
\boxed{
...
This manuscript does not claim a radiatively complete precision theory. PR-III is reserved for the derivation and precision tests that remain open: the scalar Hessian, electromagnetic and electroweak running, uniform QCD matching, and the construction of explicit flavor mass operators. Those calculations will determine which PR-II closure candidates survive as consequences of the underlying architecture.

Ultimately, the significance of PR-II lies not in overturning standard gauge theory but in anchoring it. The framework preserves the established currents, anomalies, and effective low-energy structures, while showing that several quantities normally entered independently can be organized as candidate outputs of a shared compact-boundary geometry. If the remaining radiative and running obligations are fulfilled, this architecture would provide a geometric organization for a substantial part of the electroweak and flavor parameter ledger. If those obligations cannot be met, the failure modes identified throughout the manuscript show where the projection program ceases to provide a consistent closure.
