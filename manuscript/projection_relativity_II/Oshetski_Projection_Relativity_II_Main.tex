...
\end{equation}
Together they reproduce the residual compact electromagnetic normalization\footnote{\textbf{Note on Computational Convergence and Framework Scope:} The numerical values reported in this manuscript (e.g., $\mu_{min}^2 \approx 3.052966743096$, $\alpha_{PR,bc}^{-1} \approx 137.036361812007$) reflect the baseline finite-difference solver resolution established in PR-I, ensuring strict version-control continuity across both works. Executing the identical geometric architecture through an upgraded, high-density spectral diagonalization ($N=10,000$ harmonic basis states) imptoves the finite-basis truncation residual, yielding a higher resolution spectral gap of $\mu_{min}^2 = 3.053016668059553$ and shifting the boundary-resolved electromagnetic normalization to $\alpha_{PR,bc}^{-1} = 137.036041304504209$ (compared to the empirical CODATA limit of $137.035999177(21)$). Because the objective of this manuscript is to establish the geometric architecture of the projection manifold rather than to exhaust arbitrary computational precision, the PR-I baseline is retained for all formal derivations. The high-precision limit demonstrates that the framework converges monotonically toward the empirical value as numerical resolution increases, confirming that the residual is a computational artifact rather than an architectural deviation.},
\begin{equation}
\boxed{
\alpha_{\rm PR,bc}^{-1}
=
...
\frac{1+3c_{\rm bc}}{2}
\end{equation}
is the singlet-plus-adjoint compact-boundary factor with one residual electromagnetic singlet channel plus three weak adjoint directions. The corrected hierarchy action is
\begin{equation}
\boxed{
...
\end{equation}
We find that the same overlap quantities that generate fermion mass candidates also determine the corresponding tree-level scalar coupling ledger. For the charged-lepton sector, this means
\begin{equation}
\boxed{
g_{hee}^{\rm PR}
=
...
\end{equation}
If the overlap mass ledger cannot be mapped directly to these mass-proportional vertices, the low-energy scalar sector is structurally incomplete.
...
While the three-sheet ledger successfully derives the generation count, it does not by itself explain the vast mass disparity observed between those generations. In the standard paradigm, these scale differences are accommodated by inserting arbitrary Yukawa couplings. The flavor hierarchy emerge from the boundary geometry. This hierarchical scaling is provided by the determinant suppression ratio inherited from PR-I. Functioning as a geometric attenuation factor, this ratio sequentially scales the terminal matter displacement overlap across the three distinct generation sheets, deriving a ordered mass hierarchy without relying on empirical fermion masses as inputs. The hierarchy is generated by the determinant suppression ratio inherited from PR-I:
\begin{equation}
\boxed{
r_G
=
...
With the compact-boundary strong coupling and the heavy-quark threshold scales established, the PR-II architecture must enforce the standard perturbative continuity conditions across each flavor boundary. By driving the three-loop QCD running ledger downward from the electroweak neutral matching surface, the framework uniquely isolates the flavor-dependent QCD scale parameters, \(\Lambda_{n_f}^{\rm PR}\). This mathematical descent operates on the preceding geometric derivations and guarantees seamless coupling continuity at each derived heavy-quark threshold:
\begin{equation}
\boxed{
...
\end{equation}
The unresolved confinement projector is
\begin{equation}
\boxed{
P_{\rm conf}
=
...
Evaluating the effective mass directly from these, structurally derived phases yields a definitive theoretical prediction:
\begin{equation}
\boxed{\boxed{
m_{\beta\beta}^{\rm PR}
=
