...
The second part of the manuscript asks whether the numerical electroweak scales can be tied to compact boundary data. The weak mixing angle is treated as a compact boundary-deficit effect. The electroweak scale is then generated through a spectral hierarchy action built from the PR-I boundary constants and the reduced Planck scale. This produces a zero-new-parameter weak-scale candidate:
\begin{equation}
\boxed{
v_{\rm EW}^{\rm PR}
=
...
PR-II is therefore not presented as a completed electroweak precision theory. It is presented as a test-driven compact-boundary closure program whose assumptions, generated outputs, diagnostic comparisons, and remaining derivation obligations are stated separately. The central claim is narrower: a substantial part of the tree-level electroweak and flavor ledger can be organized from the PR compact-boundary data without fitting the target observables directly.
...
The projection-locking mechanism not only isolates the unbroken electromagnetic boundary but also dictates the dynamics of the broken compact directions. Coupling the fundamental weak order scale and the Fermi constant directly from the compact spectral hierarchy, the mathematical framework accounts for the scalar degrees of freedom associated with this locked boundary.We formalize the compact order field, isolating the single physical scalar order-mode excitation and defining its tree-level couplings to both the massive weak projection modes and the fermion ledger. By evaluating the compact order kinetic term, it establishes that the same geometric constraints dictating the gauge-boson masses inherently fix their corresponding scalar interaction ratios. The previous section generated the weak order scale,
\begin{equation}
\boxed{
v_{\rm EW}^{\rm PR}
=
...
The preceding derivations collectively establish the foundational electroweak architecture, demonstrating that the gauge symmetries, mass matrices, physical currents, and interaction vertices emerge as geometric consequences of the compact non-Abelian phase projection.We also demarcates the strict boundaries of the current manuscript by explicitly cataloging the radiative, running, and higher-order precision tasks deferred to the subsequent PR-III program.
...
\begin{equation}
\boxed{
$m_h$ \space\text{is not a generation input in this manuscript}.
}
\end{equation}
...
\label{sec:pr2_charged_neutral_current_recovery}
 ...
 The weak interactions are the physical currents induced by state-motion along the $SU(2)_L$ fiber \cite{FeynmanGellMann1958}. We now demonstrates that the charged-current and neutral-current Lagrangians are the low-energy contact limits of the parent projection geometry. The central chain is defined as
\begin{equation}
\boxed{
SU(2)_L
\rightarrow
...
The charged-lepton sector is represented on a three-sheet square-root mass plane. PR-II proposes associating the two charged-current ladder orientations with the nine ordered sheet-pair incidences of a three-generation ledger. This motivates the candidate phase
\begin{equation}
\theta_K^{\rm PR}
=
\frac{2}{N_{\rm gen}^2}
...
To couple $M_\ell$ to the fundamental electroweak geometry, we derive it directly from the compact-boundary ledger. This approach shows that the charged-lepton scale is a projection of the weak displacement anchor, $\Lambda_{\rm EW}^{\rm PR}$. By adjusting this anchor with the determinant suppression ratio and the weak boundary deficit, we recover the exact scale without introducing any new free parameters. The suppression ratio is:
\begin{equation}
r_G
=
\frac{c_{\rm bc}}{q_{\rm bc}}
...
Let the normalized endpoint response weights be
\begin{equation}
\boxed{
w_{\rm singlet}=1,
\qquad
...
The structural parameters defining this rotation are derived purely from the shared geometric ledger. The Cabibbo sheet angle is defined by the boundary and confinement traces:
\begin{equation}
\boxed{
\lambda_{\rm PR}
=
...
This result should be interpreted as a PMNS lepton-mixing structure candidate. It derives large solar mixing, large atmospheric mixing, a small nonzero reactor angle, and a nonzero leptonic CP phase from compact-boundary data. It does not yet derive neutrino mass eigenvalues, mass-squared splittings, ordering, or Majorana phases. Those are completed in the neutrino mass-sector closure in Section~\ref{sec:pr2_neutrino_mass_majorana_boundary}.
...
The derivation of the CKM and PMNS matrices from the shared geometric ledger is falsifiable. The following failure modes are identified. First, naively substituting the raw determinant ratio as the Cabibbo angle fundamentally misaligns the projection and yields the incorrect CKM hierarchy. Utilizing
\begin{equation}
\lambda \neq r_G
\end{equation}
fails the structural evaluation. The architecture mandates the fully resolved compact Cabibbo sheet angle:
\begin{equation}
\lambda_{\rm PR}
=
\sqrt{
...
We now structurally project the neutrino mass spectrum. In standard phenomenological models, the mass scale and the ordering of the states (normal versus inverted) remain undetermined free parameters. Within the framework, however, the mass spectrum is a geometric consequence of the boundary architecture. The topological isolation of the residual compact-singlet sheet requires that the lowest generation state is identically massless:
\begin{equation}
m_1^{\rm PR} = 0.
\end{equation}

Because the higher-generation states receive non-zero geometric projections, this massless anchor breaks the ordering degeneracy. The generated states enforce a normal-ordered hierarchy:
\begin{equation}
...
We now formally categorize the physical mechanism driving these masses. In standard phenomenology, the choice between Dirac and Majorana states represents an unresolved, parameter-dependent binary. Within this framework, however, this classification is not an arbitrary model-building choice. Because the overlap geometry intrinsically lacks the mechanism to produce the extreme $10^{-13}$ numerical suppression required for a pure Dirac channel, the architecture rejects it. Instead, the Majorana-dominant nature of the neutrino sector is a structural consequence \cite{Weinberg1967}, governed directly by the natural dimension-five weak-order scale emerging from the compact-singlet projection. The scale
\begin{equation}
\boxed{
\frac{
(v_{\rm EW}^{\rm PR})^2
...
The geometric boundary fixes the neutrino mass spectrum, PMNS mixing angles, and relative interference phases. Together, these quantities determine the flavor-basis Majorana mass matrix:
\begin{equation}
\boxed{
M_\nu^{\rm PR}
=
...
\begin{itemize}
    ...
    \item \textbf{The Mass and Mixing Ledger:} The extreme hierarchies of the charged leptons and quarks, alongside unitary CKM and PMNS mixing candidates, are generated directly from determinant ratios, weak boundary deficits, and representation overlaps.
    \item \textbf{The Neutrino Sector:} The mass scale is pushed to the sub-eV regime by a native dimension-five topological suppression, enforcing a normal-ordered, Majorana-dominant spectrum and yielding a single compact-boundary candidate for neutrinoless double-beta decay.
\end{itemize}
